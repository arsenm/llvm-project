...
; CHECK: Function Attrs: noconvergent nofree norecurse nosync nounwind willreturn memory(argmem: write)
...
; CGSCC-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[VAL]], <4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>) #[[ATTR16]]
; CGSCC-NEXT:    ret void
;
...
; TUNIT-NEXT:    [[RES:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>, <4 x i32> undef) #[[ATTR15]]
...
; CGSCC-NEXT:    [[RES:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>, <4 x i32> undef) #[[ATTR16]]
...
; TUNIT: Function Attrs: noconvergent nounwind memory(argmem: read)
; TUNIT-LABEL: define {{[^@]+}}@test11_2
; TUNIT-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR8:[0-9]+]] {
; TUNIT-NEXT:    [[RES:%.*]] = call <4 x i32> @test11_1(<4 x ptr> [[PTRS]]) #[[ATTR16:[0-9]+]]
; TUNIT-NEXT:    ret <4 x i32> [[RES]]
;
; CGSCC: Function Attrs: noconvergent nounwind memory(argmem: read)
; CGSCC-LABEL: define {{[^@]+}}@test11_2
; CGSCC-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR9:[0-9]+]] {
; CGSCC-NEXT:    [[RES:%.*]] = call <4 x i32> @test11_1(<4 x ptr> [[PTRS]]) #[[ATTR17:[0-9]+]]
; CGSCC-NEXT:    ret <4 x i32> [[RES]]
;
...
; TUNIT-NEXT:    [[RES:%.*]] = call <4 x i32> @test12_1(<4 x ptr> [[PTRS]]) #[[ATTR16]]
; TUNIT-NEXT:    ret <4 x i32> [[RES]]
;
...
; CGSCC-NEXT:    [[RES:%.*]] = call <4 x i32> @test12_1(<4 x ptr> [[PTRS]]) #[[ATTR17]]
; CGSCC-NEXT:    ret <4 x i32> [[RES]]
;
...
; TUNIT-NEXT:    call void @byval_no_fnarg(ptr nocapture nofree readonly byval(i8) [[READ_ONLY]]) #[[ATTR16]]
; TUNIT-NEXT:    ret void
;
...
; CGSCC-NEXT:    call void @byval_no_fnarg(ptr noalias nocapture nofree noundef nonnull readnone byval(i8) dereferenceable(1) [[READ_ONLY]]) #[[ATTR17]]
; CGSCC-NEXT:    ret void
;
...
; TUNIT-NEXT:    [[CALL_PTR:%.*]] = call ptr @maybe_returned_ptr(ptr readonly [[PTR]]) #[[ATTR16]]
; TUNIT-NEXT:    [[CALL_VAL:%.*]] = call i8 @maybe_returned_val(ptr readonly [[CALL_PTR]]) #[[ATTR16]]
; TUNIT-NEXT:    ret void
;
...
; CGSCC-NEXT:    [[CALL_PTR:%.*]] = call ptr @maybe_returned_ptr(ptr readonly [[PTR]]) #[[ATTR17]]
; CGSCC-NEXT:    [[CALL_VAL:%.*]] = call i8 @maybe_returned_val(ptr readonly [[CALL_PTR]]) #[[ATTR17]]
; CGSCC-NEXT:    ret void
;
...
;.
; TUNIT: attributes #[[ATTR0]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(write) }
...
; TUNIT: attributes #[[ATTR3]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(argmem: write) }
; TUNIT: attributes #[[ATTR4:[0-9]+]] = { nocallback noconvergent nofree nosync nounwind willreturn memory(write) }
; TUNIT: attributes #[[ATTR5:[0-9]+]] = { nocallback noconvergent nofree nosync nounwind willreturn memory(read) }
; TUNIT: attributes #[[ATTR6]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(read) }
; TUNIT: attributes #[[ATTR7:[0-9]+]] = { nounwind memory(argmem: read) }
; TUNIT: attributes #[[ATTR8]] = { noconvergent nounwind memory(argmem: read) }
; TUNIT: attributes #[[ATTR9:[0-9]+]] = { nounwind memory(argmem: readwrite) }
; TUNIT: attributes #[[ATTR10]] = { noconvergent nounwind memory(argmem: readwrite) }
; TUNIT: attributes #[[ATTR11]] = { noconvergent nofree norecurse nounwind willreturn memory(argmem: readwrite) }
; TUNIT: attributes #[[ATTR12]] = { memory(none) }
; TUNIT: attributes #[[ATTR13:[0-9]+]] = { nounwind memory(read) }
; TUNIT: attributes #[[ATTR14]] = { noconvergent nounwind memory(read) }
; TUNIT: attributes #[[ATTR15]] = { willreturn }
; TUNIT: attributes #[[ATTR16]] = { nounwind }
;.
; CGSCC: attributes #[[ATTR0]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(write) }
...
; CGSCC: attributes #[[ATTR3]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(argmem: write) }
; CGSCC: attributes #[[ATTR4]] = { noconvergent nofree nosync nounwind willreturn memory(write) }
; CGSCC: attributes #[[ATTR5:[0-9]+]] = { nocallback noconvergent nofree nosync nounwind willreturn memory(write) }
; CGSCC: attributes #[[ATTR6:[0-9]+]] = { nocallback noconvergent nofree nosync nounwind willreturn memory(read) }
; CGSCC: attributes #[[ATTR7]] = { noconvergent nofree norecurse nosync nounwind willreturn memory(read) }
; CGSCC: attributes #[[ATTR8:[0-9]+]] = { nounwind memory(argmem: read) }
; CGSCC: attributes #[[ATTR9]] = { noconvergent nounwind memory(argmem: read) }
; CGSCC: attributes #[[ATTR10:[0-9]+]] = { nounwind memory(argmem: readwrite) }
; CGSCC: attributes #[[ATTR11]] = { noconvergent nounwind memory(argmem: readwrite) }
...
; CGSCC: attributes #[[ATTR14:[0-9]+]] = { nounwind memory(read) }
; CGSCC: attributes #[[ATTR15]] = { noconvergent nounwind memory(read) }
