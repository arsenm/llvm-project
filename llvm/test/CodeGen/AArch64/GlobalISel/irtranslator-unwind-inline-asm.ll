...
entry:
  ...
  invoke void asm sideeffect unwind "bl trap", ""() noconvergent
...
define void @test2() #0 personality ptr @__gcc_personality_v0 {
  ...
  ; CHECK-NEXT:   BL @_Unwind_Resume, csr_aarch64_aapcs, implicit-def $lr, implicit $sp, implicit $x0
  ; CHECK-NEXT:   ADJCALLSTACKUP 0, 0, implicit-def $sp, implicit $sp
  invoke void asm sideeffect "", "r"(ptr undef) noconvergent to label %a unwind label %b
a:
  ret void
b:
...
define dso_local void @test_convergent() personality ptr @__gxx_personality_v0 {
  ; CHECK-LABEL: name: test_convergent
  ; CHECK: bb.1.entry:
  ; CHECK-NEXT:   successors: %bb.2(0x40000000), %bb.3(0x40000000)
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   [[GV:%[0-9]+]]:_(p0) = G_GLOBAL_VALUE @.str.2
  ; CHECK-NEXT:   G_INVOKE_REGION_START
  ; CHECK-NEXT:   EH_LABEL <mcsymbol >
  ; CHECK-NEXT:   INLINEASM &"bl trap", 33 /* sideeffect isconvergent attdialect */
  ; CHECK-NEXT:   EH_LABEL <mcsymbol >
  ; CHECK-NEXT:   G_BR %bb.2
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.2.invoke.cont:
