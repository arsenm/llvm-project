...
bool PHIEliminationImpl::lowerBlockArgs(MachineFunction &MF,
  ...
  // computes LiveIntervals after PHI elimination, and even
  // -early-live-intervals adds the LiveIntervals pass after this one. Assert so
  // a caller that does have LiveIntervals live fails loudly here instead of
  // silently corrupting intervals; extend this to update LIS like LowerPHINode
  // does if that arises.
  ...
  // A DBG_PHI reading a block argument was emitted by finalizeDebugInstrRefs
  // (the argument had no defining instruction to point at). Record where the
  // value lands after register allocation and drop the DBG_PHI, mirroring how
  // ordinary PHIs are handled in LowerPHINode; a DBG_PHI holding only the
  // now-lowered argument register would otherwise leave a debug-only virtual
  // register behind. Done after the copies are placed so erasing the DBG_PHI
  // does not disturb the AfterPHIsIt insertion point.
  for (unsigned ArgNo = 0, E = Args.size(); ArgNo != E; ++ArgNo) {
    for (MachineInstr &DbgPHI :
         make_early_inc_range(MRI->use_instructions(Args[ArgNo]))) {
      if (!DbgPHI.isDebugPHI())
        continue;
      unsigned ID = DbgPHI.getOperand(1).getImm();
      unsigned SubReg = DbgPHI.getOperand(0).getSubReg();
      MF.DebugPHIPositions.insert({ID, MachineFunction::DebugPHIRegallocPos(
                                           &MBB, IncomingRegs[ArgNo], SubReg)});
      if (SI)
        SI->removeSingleMachineInstrFromMaps(DbgPHI);
      DbgPHI.eraseFromParent();
    }
  }
...
bool PHIEliminationImpl::isLiveOutPastPHIs(Register Reg,
  ...
  if (!LV->isLiveOut(Reg, *MBB))
    return false;

  // A SUCC_ARGS in MBB forwards Reg on the edge to a successor's block
  // arguments, the block-argument analog of a PHI operand, and LiveVariables
  // treats that forwarding read as an ordinary use in MBB. If MBB's SUCC_ARGS
  // are Reg's only uses, it is not live out past them - exactly as a value used
  // only in a PHI is not live out - so splitting the edge would be pointless.
  for (const MachineInstr &UseMI : MRI->use_nodbg_instructions(Reg))
    if (!UseMI.isSuccArgs() || UseMI.getParent() != MBB)
      return true;
  return false;
}
