...
MachineBasicBlock *MachineBasicBlock::SplitCriticalEdge(
  ...
  // (ReplaceUsesOfBlockWith below only rewrites terminator operands, so it
  // would not retarget the SUCC_ARGS' successor operand regardless of
  // ordering.)
  if (MachineInstr *SA = findSuccArgs(Succ)) {
    ...
    // exist solely before PHI elimination, where neither is live. Assert the
    // invariant so a caller that splits a block-argument edge with LIS live
    // fails loudly instead of silently corrupting intervals.
    assert(!LIS && "SUCC_ARGS relocation does not update LiveIntervals");
    SA->removeFromParent();
    NMBB->insert(NMBB->end(), SA);
  }
