...
void LiveVariables::recomputeForSingleDefVirtReg(Register Reg) {
  ...
  // its owning block. Use getDefBlock so this works for both a single machine
  // def and a block-argument def.
  bool IsBlockArg = MRI->isBlockArgDef(Reg);
  MachineInstr *DefMI = IsBlockArg ? nullptr : MRI->getUniqueVRegDef(Reg);
  MachineBasicBlock &DefBB = *MRI->getDefBlock(Reg);
