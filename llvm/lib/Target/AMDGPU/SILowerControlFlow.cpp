...
bool SILowerControlFlow::removeMBBifRedundant(MachineBasicBlock &MBB) {
  ...
  // Any virtual register with a live segment touching MBB's slot range has a
  // range that spans (or borders) the block being removed; once MBB is gone
  // those ranges must be recomputed from scratch. Collect them now, while MBB
  // and its slot indexes are still valid; the recompute happens after MBB is
  // erased so it reflects the final CFG.
  ...
  if (LIS) {
    ...
    for (unsigned I = 0, E = MRI->getNumVirtRegs(); I != E; ++I) {
      ...
      if (!LIS->hasInterval(Reg))
        continue;
      // Recompute any interval with a segment reaching into or bordering MBB's
      // slot range (closed overlap, so a segment ending at MBB's start - a
      // live-out into MBB - is included).
      const LiveInterval &LI = LIS->getInterval(Reg);
      for (const LiveRange::Segment &S : LI)
        if (S.start <= EndIdx && StartIdx <= S.end) {
    ...
    // Merge MBB's slot range into its predecessor and drop it from the index
    // maps before it is erased, so no stale reference to the freed block
    // remains.
  ...
  // Recompute the intervals that spanned the removed block now that the CFG and
  // slot indexes are final.
  if (LIS) {
