...
insertUndefLaneMask(MachineBasicBlock *MBB, MachineRegisterInfo *MRI,
  ...
  // Insert before the SUCC_ARGS cluster as well as the terminators.
  BuildMI(*MBB, MBB->getFirstSuccArgs(), {}, TII->get(AMDGPU::IMPLICIT_DEF),
          UndefReg);
  return UndefReg;
}
...
AMDGPU::PhiLoweringHelper::getSaluInsertionAtEnd(MachineBasicBlock &MBB) const {
  ...
  auto InsertionPt = MBB.getFirstSuccArgs();
